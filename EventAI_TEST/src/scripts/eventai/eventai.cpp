...
struct npc_kobold_minerAI : public ScriptedAI
{
	...
	void EnterCombat(Unit *pWho)
	{
		DoScriptText(SAY_VERMIN1, me, pWho);
	}
...
struct npc_longsnoutAI : public ScriptedAI
{
	npc_longsnoutAI(Creature *c) : ScriptedAI(c) {Reset();}
...
CreatureAI* GetAI_npc_longsnout(Creature *pCreature)
{
	return new npc_longsnoutAI(pCreature);
}
...
struct npc_defias_pathstalkerAI : public ScriptedAI
{
	npc_defias_pathstalkerAI(Creature *c) : ScriptedAI(c) {Reset();}
...
void AddSC_eventai()
{
	...
	newscript = new Script;
	newscript->Name="npc_longsnout";
	newscript->GetAI = &GetAI_npc_longsnout;
	newscript->RegisterSelf();
}
